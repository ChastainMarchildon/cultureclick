...
exports.admin = async (req, res) => {
  ...
  const surveys = await Survey.find({ organization: req.user.organization }).sort({
    title: "asc"
  });
...
exports.viewSurvey = (req, res) => {
  Survey.find({ surveyID: req.params.surveyid }, (err, survey) => {
    if (err) {
      console.log(err);
    } else {
      console.log(survey);
      var lookupID = req.params.surveyid;
      lookupID.toString();
      Question.find({ surveyID: lookupID }, (err, questions) => {
        if (err) {
          console.log(err);
